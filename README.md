...
> 最近三次更新:
> - 2026-03-08: 更新 dual 模式网段为 `172.31.0.0/24`，避免与历史环境冲突。
...
Docker 内部网络 devnet (172.31.0.0/24)
  ├── openwrt-server  172.31.0.10  wg0: 10.10.58.1/24
  └── openwrt-peer    172.31.0.20  wg0: 10.10.58.2/24
```
...
| `WG_SERVER_IP` | `172.31.0.10` | peer 模式下 server 容器的 devnet IP |
...
docker exec -it openwrt-peer /bin/ash
# （容器内，172.31.0.10 是 server 的 devnet IP）
phantun-client -l 127.0.0.1:51820 -r 172.31.0.10:4567 &
...
# peer 容器内
udp2raw -c -l 0.0.0.0:3333 -r 172.31.0.10:4096 -k "testpasswd" --raw-mode faketcp &
