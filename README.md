...
for plugin_dir in /luci-plugins/luci-app-*/; do
    ...
    # root/ 覆盖：UCI 配置、init.d 脚本、可执行文件等
...
```bash
./dev.sh status       # 查看容器状态、网络接口列表、WireGuard 状态
./dev.sh log          # 实时查看容器日志（tail -f）
./dev.sh shell        # 进入容器 ash/bash shell
./dev.sh ssh          # SSH 登录容器（等同于 ssh root@localhost -p 2222）
./dev.sh reinit       # 强制重新安装所有 opkg 包（清除初始化标记后重启）
```
...
# 检查 uhttpd 是否运行
docker exec openwrt-luci-devbox pidof uhttpd

# 手动重启 uhttpd
docker exec openwrt-luci-devbox /etc/init.d/uhttpd restart
```
...
```bash
# 检查容器是否能访问外网
docker exec openwrt-luci-devbox curl -I https://downloads.openwrt.org
...
# 如果还没变化，强制清除所有缓存
docker exec openwrt-luci-devbox rm -rf /tmp/luci-* /tmp/*.luac
./dev.sh reload
```
