...
> 标题: openwrt-luci-devbox README
> 作者: reyan
> 日期: 2026-03-08
> 版本: 1.2.0
> 描述: OpenWrt LuCI DevBox 的开发、单容器与双容器联调说明文档。
> 最近三次更新:
> - 2026-03-09: 更新 dual 模式网段为 `172.41.0.0/24`，避免与 docker-app-phantun 联调网段冲突。
> - 2026-03-08: 补充 WireGuard 检测误报修复、firewall/log 启动链修复说明。
> - 2026-03-08: 说明 peer 容器当前无健康检查，running 为设计现状。
...
#### 4. WireGuard 检测与 firewall 运行态说明

- 当前 devbox 会以 **是否成功创建 `type wireguard` 接口** 作为判断依据，而不是单看 `modprobe wireguard` 是否成功；
- 这可以避免“宿主机已支持 WireGuard，但容器内 `modprobe` 失败”时产生误报；
- `firewall4` 与 `logd` 会在启动链中显式拉起，以减少 LuCI 页面对 firewall state / 日志对象的异常提示；
- dual 模式下 peer 角色不会再为 `wg0` 绑定监听端口，避免与 phantun client 默认本地 UDP 端口冲突；
- dual 模式下目前只有 `openwrt-server` 配置了 healthcheck，`openwrt-peer` 为 `running` 属于当前设计行为，不代表异常。
...
Docker 内部网络 devnet (172.41.0.0/24)
  ├── openwrt-server  172.41.0.10  wg0: 10.10.58.1/24
  └── openwrt-peer    172.41.0.20  wg0: 10.10.58.2/24
```
...
| `WG_LISTEN_PORT` | `51820` | WireGuard 监听端口（server 模式主要使用） |
| `WG_SERVER_IP` | `172.41.0.10` | peer 模式下 server 容器的 devnet IP |
...
docker exec -it openwrt-peer /bin/ash
# （容器内，172.41.0.10 是 server 的 devnet IP）
phantun-client -l 127.0.0.1:51820 -r 172.41.0.10:4567 &
...
# peer 容器内
udp2raw -c -l 0.0.0.0:3333 -r 172.41.0.10:4096 -k "testpasswd" --raw-mode faketcp &
